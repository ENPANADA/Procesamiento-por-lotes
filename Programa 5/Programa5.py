...
"}")
        ...
        self.verticalLayout_7 = QtWidgets.QVBoxLayout(self.page_Procesos)
        self.verticalLayout_7.setContentsMargins(0, 0, 0, 0)
        self.verticalLayout_7.setSpacing(10)
        self.verticalLayout_7.setObjectName("verticalLayout_7")
        spacerItem10 = QtWidgets.QSpacerItem(17, 6, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
...
"")
        ...
        self.tableWidget.setMinimumSize(QtCore.QSize(287, 350))
        ...
        spacerItem20 = QtWidgets.QSpacerItem(17, 11, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        ...
        spacerItem21 = QtWidgets.QSpacerItem(20, 9, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        ...
        self.lineEdit_3.textChanged.connect(lambda: self.EstadoInicial3())
        self.lineEdit_2.editingFinished.connect(lambda: self.pushButton_2_Enter())
        self.lineEdit_3.editingFinished.connect(lambda: self.pushButton_2_Enter())
    ...
    def pushButton_2_Enter(self):
        self.pushButton_2.click()
    def PesstañaAsignar_Boton(self):
        global Programadores,Quantum
        bandera=False
        if Programadores!=0:
        ...
        try:
            ...
            bandera=True
        try:
            Cebo2=int(self.lineEdit_3.text())
        except:
            self.lineEdit_3.setStyleSheet("border-bottom:2px solid red;")
            bandera=True
        if bandera:
            return
        if Cebo<1 or Cebo>100:
    ...
    def FabricaDeMinioms(self,x):
        if x>100:
            return
        Operadores=['+','-','*','/','%']
        Operacion = ('{}{}{}'.format(random.randint(-100, 100),random.choice(Operadores),random.randint(1, 100)))
        SubLote=[x,random.randint(5, 16),-1,-1,-1,-1,0,0,Operacion,('{}'.format(int(eval(Operacion)))),'Nuevo']
        Lotes.append(SubLote)
    def GeneradorDeMinioms(self):
        global Programadores
        for x in range(Programadores):
            self.FabricaDeMinioms(x+1)
        self.stackedWidget.setCurrentWidget(self.page_Procesos)
        self.LlenadoDeDatos()
    ...
    def aMimir(self,time):
        ...
        loop.exec_()
    def Incremento_TE(self,x):
        for i in range(len(SubLotesPendientes)):
            if SubLotesPendientes[i][1]!=x:
                Lotes[SubLotesPendientes[i][1]][6]+=1
    def ProcesarDatos(self,x):
        global BanderaEjecucion,Bandera_Error,Bandera_P_C,BanderaInterrupcion,ContadorGlobal,Quantum
        BanderaEjecucion=True
        if Lotes[x][5]==-1:
            Lotes[x][5]=ContadorGlobal-Lotes[x][2]
        Lotes[x][10]='Ejecucion'
        ...
        self.lineEdit_27.setText('{}'.format(Quantum))
        self.aMimir(500)
        ...
        for i in range(Lotes[x][7],Lotes[x][1]):
            ...
            self.lineEdit_27.setText('{}'.format(Quantum-QuantumAUX-1))
            Lotes[x][7]+=1
            self.Incremento_TE(x)
            self.ContadorGlobal()
        ...
        if Bandera_Error:
            Lotes[x][9]='ERROR'
        Lotes[x][10]='Finalizado'
        self.ProcesosFinalizados(Lotes[x][0],Lotes[x][1],Lotes[x][2],Lotes[x][3],Lotes[x][4],Lotes[x][5],Lotes[x][6],Lotes[x][7],Lotes[x][8],Lotes[x][9])
        self.LlenarTablaBCP()
        self.aMimir(500)
        return False
    ...
    def LlenadoDeDatos(self):
        global Programadores,BanderaProcesos,CantProcesosBloqueados,ContadorGlobal,ContadorMinioms,BanderaInterrupcion
        BanderaProcesos=False
        self.lineEdit_21.setText('{}'.format(Programadores))
        ...
        if ContadorMinioms<Programadores:
            ...
            Lotes[ContadorMinioms][10]='Listo'
            ContadorMinioms+=1
            self.lineEdit_21.setText('{}'.format(Programadores-ContadorMinioms))
            self.aMimir(500)
            if ContadorMinioms<Programadores:
                Lotes[ContadorMinioms][2]=ContadorGlobal
            ...
            if SubLotesPendientes[0][0]==1:
                ...
                if self.ProcesarDatos(SubLotesPendientes[0][1]):
                    if BanderaInterrupcion:
                        ...
                        self.SalidaDelAnexo(SubLotesPendientes[0][0],SubLotesPendientes[0][1])
        ...
        self.lineEdit_26.setStyleSheet("border-bottom:2px solid rgb(225, 131, 0);")
    #PaginaFinalizados
    def ProcesosFinalizados(self,ID,TME,TL,TF,TRo,TRa,TE,TS,Operacion,Resultado):
        ...
        item = QtWidgets.QTableWidgetItem('{}'.format(Resultado))
        if Bandera_Error:
            brush = QtGui.QBrush(QtGui.QColor(255, 0, 0))
            brush.setStyle(QtCore.Qt.FDiagPattern)
            item.setBackground(brush)
    ...
    def LlenarTablaBCP(self):
        Minioms = len(Lotes)
        self.tableWidget_3.setRowCount(Minioms)
        for i in range(Minioms):
            item = QtWidgets.QTableWidgetItem('{}'.format(Lotes[i][0]))
            item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.tableWidget_3.setItem(i, 0, item)
            item = QtWidgets.QTableWidgetItem(Lotes[i][10])
            item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.tableWidget_3.setItem(i, 1, item)
            item = QtWidgets.QTableWidgetItem('Null')
            if Lotes[i][2]!=-1:
                item = QtWidgets.QTableWidgetItem('{}'.format(Lotes[i][2]))
            item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.tableWidget_3.setItem(i, 2, item)
            item = QtWidgets.QTableWidgetItem('Null')
            if Lotes[i][3]!=-1:
                item = QtWidgets.QTableWidgetItem('{}'.format(Lotes[i][3]))
            item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.tableWidget_3.setItem(i, 3, item)
            item = QtWidgets.QTableWidgetItem('Null')
            if Lotes[i][4]!=-1:
                item = QtWidgets.QTableWidgetItem('{}'.format(Lotes[i][4]))
            item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.tableWidget_3.setItem(i, 4, item)
            item = QtWidgets.QTableWidgetItem('Null')
            if Lotes[i][5]!=-1:
                item = QtWidgets.QTableWidgetItem('{}'.format(Lotes[i][5]))
            item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.tableWidget_3.setItem(i, 5, item)
            item = QtWidgets.QTableWidgetItem('Null')
            if Lotes[i][10]!='Nuevo':
                item = QtWidgets.QTableWidgetItem('{}'.format(Lotes[i][6]))
            item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.tableWidget_3.setItem(i, 6, item)
            item = QtWidgets.QTableWidgetItem('Null')
            if Lotes[i][10]!='Nuevo':
                item = QtWidgets.QTableWidgetItem('{}'.format(Lotes[i][7]))
            item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.tableWidget_3.setItem(i, 7, item)
            item = QtWidgets.QTableWidgetItem('{}'.format(Lotes[i][1]))
            item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.tableWidget_3.setItem(i, 8, item)
            item = QtWidgets.QTableWidgetItem(Lotes[i][8])
            item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.tableWidget_3.setItem(i, 9, item)
            item = QtWidgets.QTableWidgetItem('Null')
            if Lotes[i][10] =='Finalizado':
                item = QtWidgets.QTableWidgetItem(Lotes[i][9])
            item.setTextAlignment(QtCore.Qt.AlignCenter)
    ...
    def PlazaLibre(self):
        ...
        Plaza3=True
        for i in range(len(SubLotesPendientes)):
            if 1==abs(SubLotesPendientes[i][0]):
                Plaza1=False
            elif 2==abs(SubLotesPendientes[i][0]):
                Plaza2=False
            elif 3==abs(SubLotesPendientes[i][0]):
                Plaza3=False
        if Plaza1:
            return 1
        if Plaza2:
    ...
    def RestablecerValores(self):
        global Programadores,ContadorGlobal,BanderaProcesos,LotesTerminados,Pos_lote,BanderaEjecucion,Bandera_Error,Bandera_P_C,BanderaInterrupcion,CantidadLotesTerminados,ContadorMinioms,Quantum
        if BanderaProcesos:
            Programadores=0
            ContadorMinioms=0
            ContadorGlobal=0
    ...
    def CapturarTeclas(self,event):
        global BanderaEjecucion,Bandera_Error,Bandera_P_C,BanderaInterrupcion,Programadores,ContadorMinioms
        if BanderaEjecucion:
            ...
            elif event.text()=='n' or event.text()=='N':
                Programadores+=1
                self.FabricaDeMinioms(Programadores)
                self.lineEdit_21.setText('{}'.format(Programadores-ContadorMinioms))
                if len(SubLotesPendientes)<4:
                    self.NuevoBecarios(self.PlazaLibre())
            elif event.text()=='t' or event.text()=='T':
                Bandera_P_C=True
                self.MostrarPestañaTiempos()
