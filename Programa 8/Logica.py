...
def PagarMinita(self, sateo):
    for plaza in self.Marcos:
        if plaza[0] == sateo[self.getPos['ID']]:
            plaza[0] = 0
            plaza[1] = 0
            plaza[2] = 0
def TirarADN(self):
    ...
    for row in range(self.tableWidget_6.rowCount()):
        for col in range(self.tableWidget_6.columnCount()//2):
            if self.Marcos[NumToto][0] != 0 and self.Marcos[NumToto][0] != -1:
                item = QtWidgets.QTableWidgetItem('█' * self.Marcos[NumToto][2])
                if self.Lotes[self.Marcos[NumToto][0]-1][self.getPos['Estado']] == 'Listo':
                    brush = QtGui.QBrush(QtGui.QColor(225, self.Marcos[NumToto][1], 0))
                elif self.Lotes[self.Marcos[NumToto][0]-1][self.getPos['Estado']] == 'Ejecucion':
                    brush = QtGui.QBrush(QtGui.QColor(0, 255, 0))
                elif self.Lotes[self.Marcos[NumToto][0]-1][self.getPos['Estado']] == 'Bloqueado':
...
def PlantarSemilla(self, sateo):
    ...
    for toto in self.Marcos:
        if toto[0] == 0:
            toto[0] = sateo[self.getPos['ID']]
            toto[1] = ColorDelToto
            toto[2] = TotosALlenar[0]
            TotosALlenar.pop(0)
            if len(TotosALlenar) == 0:
                TirarADN(self)
                return
def DarDeAltaBecario(self):
    GatasDespachadas = self.tableWidget_5.rowCount() + self.tableWidget.rowCount() + self.tableWidget_2.rowCount() + len(self.Suspendidos)
    ...
    if BuscarBellakas(self,self.Lotes[GatasDespachadas][self.getPos['Tamaño']]):
        ...
        try:
            ...
            self.lineEdit_21.setText(str(len(self.Lotes) - (self.tableWidget_5.rowCount() + self.tableWidget.rowCount() + self.tableWidget_2.rowCount() + len(self.Suspendidos))))
            return True
        except:
...
def IniciarCorrida(self):
    ...
    ActualizarStatusMigracion(self)
    bandera = True
    while bandera:
        bandera = DarDeAltaBecario(self)
        aMimir(self,300)
    Victima = 0
    while self.tableWidget_5.rowCount() != 0 or self.tableWidget_2.rowCount() != 0 or len(self.Suspendidos) != 0:
...
def EsclavizarMiniom(self,ProcesoEnEjecucion):
    ...
    PagarMinita(self,ProcesoEnEjecucion)
    TirarADN(self)
...
def VerMercadoSexual(self):
    for celda in range(self.tableWidget_4.rowCount()-1):
        ...
        if turro == 0:
            ...
            item = QtWidgets.QTableWidgetItem('██' * self.Marcos[celda * 2][2])
            if Estado == 'Listo':
                brush = QtGui.QBrush(QtGui.QColor(225, self.Marcos[celda * 2][1], 0))
        ...
        if turro == 0:
            ...
            item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.tableWidget_4.setItem(celda, 5, item)
            item = QtWidgets.QTableWidgetItem('██' * self.Marcos[celda * 2 + 1][2])
            if Estado == 'Listo':
                brush = QtGui.QBrush(QtGui.QColor(225, self.Marcos[celda * 2 + 1][1], 0))
            elif Estado == 'Ejecucion':
                brush = QtGui.QBrush(QtGui.QColor(0, 255, 0))
            elif Estado == 'Bloqueado':
                brush = QtGui.QBrush(QtGui.QColor(255, 0, 0))
...
def LlenarTablaBCP(self):
    ...
    for i in range(self.tableWidget_3.rowCount()):
        Estado = str(self.Lotes[i][self.getPos['Estado']])
...
def ActualizarStatusMigracion(self):
    if len(self.Suspendidos) == 0:
        self.lineEdit_25.setText('0')
        self.lineEdit_26.setText('Null')
        self.lineEdit_28.setText('Null')
    else:
        self.lineEdit_25.setText(str(len(self.Suspendidos)))
        self.lineEdit_26.setText(str(self.Suspendidos[0][self.getPos['ID']]))
        self.lineEdit_28.setText(str(self.Suspendidos[0][self.getPos['Tamaño']]))
def ReporteDeInmigraciones(self):
    with open('Reporte_Suspendidos.txt','w') as deportados:
        text = 'No. Procesos Suspendidos: ' + str(len(self.Suspendidos)) + '\n'
        for mojado in self.Suspendidos:
            text += ( '\n\tID: ' + str(mojado[self.getPos['ID']])
                    + '\n\tTME: ' + str(mojado[self.getPos['TME']])
                    + '\n\tTamaño: ' + str(mojado[self.getPos['Tamaño']])
                    + '\n\tTiempo de Llegada: ' + str(mojado[self.getPos['TL']])
                    + '\n\tTiempo de Finalizacion: ' + 'Null'
                    + '\n\tTiempo de Tetorno: ' + 'Null'
                    + '\n\tTiempo de Respuesta: ' + str(mojado[self.getPos['TRa']])
                    + '\n\tTiempo de Espera: ' + str(mojado[self.getPos['TE']])
                    + '\n\tTiempo de Servicio: ' + str(mojado[self.getPos['TS']])
                    + '\n\tOperacion: ' + str(mojado[self.getPos['Operacion']])
                    + '\n\tResultado: ' + 'Null'
                    + '\n\tEstado: ' + str(mojado[self.getPos['Estado']]) + '\n')  
        deportados.write(text)
def PalGabacho(self):
    if self.tableWidget_2.rowCount() == 0:
        return 
    mojado = self.tableWidget_2.item(0,0)
    mojado = int(mojado.text())
    PagarMinita(self,self.Lotes[mojado - 1])
    self.Lotes[mojado - 1][self.getPos['Estado']] = 'Suspendido'
    self.tableWidget_2.removeRow(0)
    self.Suspendidos.append(self.Lotes[mojado - 1])
    TirarADN(self)
    ReporteDeInmigraciones(self)
    ActualizarStatusMigracion(self)
    DarDeAltaBecario(self)
def DeportarMexas(self):
    if len(self.Suspendidos) == 0:
        return
    if BuscarBellakas(self,self.Suspendidos[0][self.getPos['Tamaño']]):
        self.Lotes[self.Suspendidos[0][self.getPos['ID']]-1][self.getPos['Estado']] = 'Listo'
        PlantarSemilla(self, self.Suspendidos[0])
        RehabilitarRecluso(self, self.Suspendidos[0])
