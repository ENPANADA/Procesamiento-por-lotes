...
def DarDeAltaBecario(self):
    GatasDespachadas = self.tableWidget_5.rowCount() + self.tableWidget.rowCount()
    ...
    if BuscarBellakas(self,self.Lotes[GatasDespachadas][self.getPos['Tamaño']]):
        ...
        try:
            ...
            self.lineEdit_21.setText(str(len(self.Lotes) - (self.tableWidget_5.rowCount() + self.tableWidget.rowCount())))
            return True
        except:
...
def IniciarCorrida(self):
    ...
    while self.tableWidget_5.rowCount() != 0:
        ...
        if self.tableWidget_2.rowCount() == self.tableWidget_5.rowCount():
        ...
        if self.Lotes[ID_Victima][self.getPos['Estado']] == 'Bloqueado':
            Victima += 1
            continue
